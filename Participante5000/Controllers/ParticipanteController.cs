...
namespace Participante5000.Controllers;
...
public class ParticipantController : ControllerBase
{
    ...
    private readonly int _portaAtual = 5000;
    ...
    {
        Console.WriteLine($"[{_portaAtual}] Votando SIM para: {mensagem}");
        return true;
    }
    ...
    public async Task<IActionResult> IniciarTransacao([FromBody] string mensagem)
    {
        ...
        // envia requisição de voto para os outros participantes
        ...
        // faz a contabilização dos votos e escreve ou não no arquivo
        if (votos.All(v => v))
        {
            foreach (var porta in outrasPortas)
